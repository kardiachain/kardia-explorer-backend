...
import (
	...
	"github.com/kardiachain/kardia-explorer-backend/server/audit"
...
func main() {
	...
	if err != nil {
		lgr.Error("cannot create kai client", zap.Error(err))
		panic(err)

	}

	srv := new(audit.Server).
		SetLogger(lgr).
	...
	go srv.AuditBlocks(ctx, 3000000, 3200000)
	<-waitExit
	lgr.Info("Stopped")
}
