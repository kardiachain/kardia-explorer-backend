...
type IKRC721Holder interface {
	...
	KRC721Inventory(ctx context.Context, filter types.KRC721HolderFilter) ([]*types.KRC721Holder, uint64, error)
...
func (m *mongoDB) KRC721Inventory(ctx context.Context, filter types.KRC721HolderFilter) ([]*types.KRC721Holder, uint64, error) {
	var (
		holders []*types.KRC721Holder
		crit    = bson.M{}
	)
	...
	matchStage := bson.D{
		{"$match", bson.D{{"contractAddress", filter.ContractAddress}}},
	}
	groupStage := bson.D{{"$group", bson.D{{"$owner"}}}}

	cursor, err := m.wrapper.C(cKRC721Holders).Aggregate()
	if err != nil {
		return nil, 0, err
	}
