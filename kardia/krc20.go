...
func (ec *Client) GetKRC20TokenInfo(ctx context.Context, a *abi.ABI, krcTokenAddr common.Address) (*types.KRCTokenInfo, error) {
	...
	if err != nil {
		ec.lgr.Warn("Cannot get KRC20 token name", zap.String("smcAddress", krcTokenAddr.Hex()), zap.Error(err))
		return nil, err
	}
	symbol, err := ec.getKRC20TokenSymbol(ctx, a, krcTokenAddr)
	if err != nil {
		ec.lgr.Warn("Cannot get KRC20 token symbol", zap.String("smcAddress", krcTokenAddr.Hex()), zap.Error(err))
		return nil, err
	}
	totalSupply, err := ec.getKRC20TotalSupply(ctx, a, krcTokenAddr)
	if err != nil {
		ec.lgr.Warn("Cannot get KRC20 token total supply", zap.String("smcAddress", krcTokenAddr.Hex()), zap.Error(err))
		return nil, err
	}
	decimals, err := ec.getKRC20TokenDecimal(ctx, a, krcTokenAddr)
	if err != nil {
		ec.lgr.Warn("Cannot get KRC20 token decimals", zap.String("smcAddress", krcTokenAddr.Hex()), zap.Error(err))
		return nil, err
	}
