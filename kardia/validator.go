...
func (ec *Client) Validators(ctx context.Context) ([]*types.Validator, error) {
	// Get trusted node and create kardia node conn using kardia-goclient
	return ec.getValidators(ctx)
	//
	//var (
	//	proposersStakedAmount = big.NewInt(0)
	//	validators            []*types.Validator
	//)
	//err := ec.defaultClient.c.CallContext(ctx, &validators, "kai_validators", false)
	//if err != nil {
	//	return nil, err
	//}
